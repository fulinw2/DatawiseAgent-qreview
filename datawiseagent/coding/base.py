...
# define what is publicly visible whithin the module
...
class IPythonCodeResult(CodeResult):
    ...
    time_consumed: Optional[float] = Field(
        default=None, description="The time consumed when executing the code."
    )

    def __str__(self):
        output_files_str = "\n".join(self.output_files)
        if self.time_consumed:
            return f"""Time consumed: {self.time_consumed:.2f}s\nCode output:\nExit Code: {self.exit_code}\nOutput: {self.output}"""
        else:
            return (
                f"""Code output:\nExit Code: {self.exit_code}\nOutput: {self.output}"""
            )
