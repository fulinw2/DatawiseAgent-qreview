...
# CODE_BLOCK_PATTERN = r"```[ \t]*(markdown|python)?[ \t]*\r?\n(.*?)\r?\n[ \t]*```"
CODE_BLOCK_PATTERN = r"```(.*?)\r?\n(.*?)```"

LIMITED_CODE_BLOCK_PATTERN = r"```[ \t]*(\w+)?[ \t]*\r?\n(.*?)\r?\n[ \t]*```"
...
def infer_lang(code: str) -> str:
    ...
    Currently, the language is either Python or Markdown.
    """
    # 复制代码副本
    code_copy = code
    # 移除魔法指令和 Shell 命令
    filtered_lines = []
    for line in code_copy.splitlines():
        # 移除以 % 或 %% 开头的魔法指令
...
def extract_code(
    ...
    if not detect_single_line_code:
        ...
        start_pos = 0
        while True:
            match = code_pattern.search(text, pos=start_pos)
            if not match:
                break
            ...
            if code:
                ...
                extracted.append((inferred_lang, code))

            # 更新下一次搜索的起始位置
            # 将 start_pos 设置为 match.end() - 3，以确保包含上一次的 ```
            start_pos = match.end() - 3

        # 如果没有检测到代码块，返回整个文本作为未知语言
        if not extracted:
            return [(UNKNOWN, text)]
...
if __name__ == "__main__":
    text = """```markdown                                                                                                                                                                                                                  
# Debugging Attempt                                                                                                                                                                                                          
The previous attempt to make predictions on the test dataset encountered several issues. We will address these issues step by step.                                                                                          
                                                                                                                                                                                                                             
## Issues Identified                                                                                                                                                                                                         
1. **FutureWarning**: The use of `inplace=True` when filling missing values in the `loading` column is causing a warning. We will modify this to avoid using `inplace=True`.                                                 
2. **KeyError**: The test dataset does not contain the expected categorical columns (`product_code`, `attribute_0`, `attribute_1`) after the one-hot encoding step. This indicates that these columns may not exist in the te
st dataset.                                                                                                                                                                                                                  
                                                                                                                                                                                                                             
## Next Steps                                                                                                                                                                                                                
1. We will fill missing values without using `inplace=True`.                                                                                                                                                                 
2. We will check the columns of the test dataset before attempting to encode them and ensure that we only encode existing columns.                                                                                           
...
# Fill missing values in 'loading' with the mean without using inplace                                                                                                                                                       
test_data['loading'] = test_data['loading'].fillna(test_data['loading'].mean())                                                                                                                                              
                                                                                                                                                                                                                             
# Check the columns of the test data before encoding                                                                                                                                                                         
print("Test Data Columns Before Encoding:", test_data.columns.tolist())                                                                                                                                                      
                                                                                                                                                                                                                             
# Encode categorical variables using the same method as training                                                                                                                                                             
# Only encode if the columns exist in the test data                                                                                                                                                                          
columns_to_encode = ['product_code', 'attribute_0', 'attribute_1']                                                                                                                                                           
...
# Check the columns of the test data after encoding                                                                                                                                                                          
print("Test Data Columns After Encoding:", test_data.columns.tolist())                                                                                                                                                       
                                                                                                                                                                                                                             
# Align the columns of train and test data                                                                                                                                                                                   
# We need to ensure that the test data has the same columns as the model expects                                                                                                                                             
model_columns = model.feature_importances_.index                                                                                                                                                                             
