...
try:
    global_config = yaml.safe_load(open(DEFAULT_CONFIG_PATH))
except FileNotFoundError:
    logger.warn(
        "No default config file found, using empty config as the default config. "
        "If you want to use a default config file, please make sure it is in the "
        "'configs' folder and named 'default_config.yaml'"
    )
    global_config = {}
