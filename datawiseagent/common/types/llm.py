...
class LLMResult(BaseModel):
    content: None | str | dict | List[ChatCompletionContentPartParam] = ""
    # dict means the string content is parsed into JSON in json mode
    # list[dict] means List[ChatCompletionContentPartParam]
    role: str = ""
    name: str = ""
    tool_calls: list[ChatCompletionMessageToolCall] | None = None
    parsed_tool_calls: list[ChatCompletionMessageToolCall] | None = (
        None  # arguments are parsed into python objects, such as JSON
    )
    ...
    def __init__(self, **data):
        ...
        if self.tool_calls:
            ...
            for tool_call in self.tool_calls:
                ...
                tool_call_.function.arguments = json.loads(
                    JsonRepair(tool_call_.function.arguments).repair()
                )
    ...
    def __hash__(self):
        return hash(
            (
                ...
                (
                    (
                        (t.id, t.function.arguments, t.function.name)
