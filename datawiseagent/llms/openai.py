...
from datawiseagent.common.utils.misc import log_retry
...
class OpenAIChat(BaseChatModel):
        ...
        **kwargs,
        ...
        if OPENAI_RESPONSE_LOG_PATH:
            ...
            data_to_save = {
                ...
                "response_format": (
                    None if response_format is NOT_GIVEN else response_format
                ),
                "model_args": model_args,
                "response": response.model_dump(),
            }
            model_name = model_args.get("model", "openai_default")
            filename = f"{model_name}_response_{uuid4()}.json"
            try:
        ...
        if (
            response_format is not NOT_GIVEN
            ...
            if response.choices[0].message.content is not None:
                try:
                    ...
                    content = json.loads(
                        JsonRepair(response.choices[0].message.content).repair()
                    )
        ...
        if not (
            response.choices[0].message.tool_calls is None  # None
            ...
            for tool in response.choices[0].message.tool_calls:
                ...
                if not is_valid:
                    ...
                    raise ValueError(
                        f"The returned function name {tool_name} is not in the list of valid functions."
                    )
    ...
    def construct_messages(
        self, prepend_prompt: list[str], history: list[dict], append_prompt: list[str]
    ):
        messages = []
        # if prepend_prompt != []:
...
def get_embedding(text: str) -> np.array:
    try:
        ...
        if openai.api_type == "azure":
            embedding = sync_openai_client.embeddings.create(
